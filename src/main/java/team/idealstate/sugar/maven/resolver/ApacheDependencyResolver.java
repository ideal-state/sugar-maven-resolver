...
final class ApacheDependencyResolver implements DependencyResolver {
    @NotNull
    @Override
    public Dependency resolve(@NotNull String groupId, @NotNull String artifactId, @Nullable String extension, @Nullable String classifier, @NotNull String version, @NotNull DependencyScope scope) {
    ...
    public List<Dependency> resolvePom(@NotNull InputStream pomInputStream) {
        try {
            List<org.apache.maven.model.Dependency> dependencies = new MavenXpp3Reader().read(pomInputStream).getDependencies();
            if (dependencies.isEmpty()) {
                return Collections.emptyList();
            }
            return dependencies.stream().map(it -> resolve(
                    it.getGroupId(),
                    it.getArtifactId(),
                    null,
                    it.getClassifier(),
